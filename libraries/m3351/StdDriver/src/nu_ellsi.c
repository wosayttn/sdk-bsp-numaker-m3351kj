...
void ELLSI_Open(ELLSI_T *ellsi,
                ...
                uint32_t u32IDOS)
{
    S_ELLSI_CONFIG_T sELLSIConfig = {0};

    sELLSIConfig.u32ELLSIMode = u32ELLSIMode;
    sELLSIConfig.u32OutputFormat = u32OutputFormat;
    sELLSIConfig.sTimeInfo.u32BusClock = u32BusClock;
    sELLSIConfig.sTimeInfo.u32TransferTimeNsec = u32TransferTimeNsec;
    sELLSIConfig.sTimeInfo.u32T0HTimeNsec = u32T0HTimeNsec;
    sELLSIConfig.sTimeInfo.u32T1HTimeNsec = u32T1HTimeNsec;
    sELLSIConfig.sTimeInfo.u32ResetTimeNsec = u32ResetTimeNsec;
    sELLSIConfig.u32PCNT = u32PCNT;
    sELLSIConfig.u32IDOS = u32IDOS;

    ELLSI_OpenbyConfig(ellsi, &sELLSIConfig);
}
...
void ELLSI_OpenbyConfig(ELLSI_T *ellsi, S_ELLSI_CONFIG_T *sELLSIConfig)
{
    ...
    ellsi->CTL = (sELLSIConfig->u32ELLSIMode) | (sELLSIConfig->u32OutputFormat);
    ellsi->PCNT = sELLSIConfig->u32PCNT;
    ellsi->OCTL = sELLSIConfig->u32IDOS;

    if (sELLSIConfig->sTimeInfo.u32BusClock >= u32PCLKFreq)
    {
        /* Set DIVIDER = 0 */
        u32Div = 0;
        ellsi->CLKDIV = 0;
    }
    else if (sELLSIConfig->sTimeInfo.u32BusClock == 0)
    {
    ...
    {
        u32Div = (((u32PCLKFreq * 10) / sELLSIConfig->sTimeInfo.u32BusClock + 5) / 10) - 1; /* Round to the nearest integer */
    ...
    u32Period = (u32Tmp1 * sELLSIConfig->sTimeInfo.u32TransferTimeNsec / u32Tmp2 + 5) / 10; /* Round to the nearest integer */
    ...
    u32T0H = (u32Tmp1 * sELLSIConfig->sTimeInfo.u32T0HTimeNsec / u32Tmp2 + 5) / 10; /* Round to the nearest integer */
    ...
    u32T1H = (u32Tmp1 * sELLSIConfig->sTimeInfo.u32T1HTimeNsec / u32Tmp2 + 5) / 10; /* Round to the nearest integer */
    ...
    u32ResetPeriod = (u32Tmp1 * sELLSIConfig->sTimeInfo.u32ResetTimeNsec / u32Tmp2 + 5) / 10; /* Round to the nearest integer */
