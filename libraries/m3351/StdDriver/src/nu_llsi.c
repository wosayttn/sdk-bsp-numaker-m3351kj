...
void LLSI_Open(LLSI_T *llsi,
               ...
               uint32_t u32IDOS)
{
    S_LLSI_CONFIG_T sLLSIConfig = {0};

    sLLSIConfig.u32LLSIMode = u32LLSIMode;
    sLLSIConfig.u32OutputFormat = u32OutputFormat;
    sLLSIConfig.sTimeInfo.u32BusClock = u32BusClock;
    sLLSIConfig.sTimeInfo.u32TransferTimeNsec = u32TransferTimeNsec;
    sLLSIConfig.sTimeInfo.u32T0HTimeNsec = u32T0HTimeNsec;
    sLLSIConfig.sTimeInfo.u32T1HTimeNsec = u32T1HTimeNsec;
    sLLSIConfig.sTimeInfo.u32ResetTimeNsec = u32ResetTimeNsec;
    sLLSIConfig.u32PCNT = u32PCNT;
    sLLSIConfig.u32IDOS = u32IDOS;

    LLSI_OpenbyConfig(llsi, &sLLSIConfig);
}
...
void LLSI_OpenbyConfig(LLSI_T *llsi, S_LLSI_CONFIG_T *sLLSIConfig)
{
    ...
    if ((llsi == LLSI0) || (llsi == LLSI2) || (llsi == LLSI4) || (llsi == LLSI6) || (llsi == LLSI8) || (llsi == (LLSI_T *)ELLSI0))
        u32PCLKFreq = CLK_GetPCLK0Freq();
    else if ((llsi == LLSI1) || (llsi == LLSI3) || (llsi == LLSI5) || (llsi == LLSI7) || (llsi == LLSI9) || (llsi == (LLSI_T *)ELLSI1))
        u32PCLKFreq = CLK_GetPCLK1Freq();

    /* Default setting: software mode, RGB format, idle ouput low. */
    llsi->CTL = (sLLSIConfig->u32LLSIMode) | (sLLSIConfig->u32OutputFormat);
    llsi->PCNT = sLLSIConfig->u32PCNT;
    llsi->OCTL = sLLSIConfig->u32IDOS;

    if (sLLSIConfig->sTimeInfo.u32BusClock >= u32PCLKFreq)
    {
        /* Set DIVIDER = 0 */
        u32Div = 0;
        llsi->CLKDIV = 0;
    }
    else if (sLLSIConfig->sTimeInfo.u32BusClock == 0)
    {
    ...
    {
        u32Div = (((u32PCLKFreq * 10) / sLLSIConfig->sTimeInfo.u32BusClock + 5) / 10) - 1; /* Round to the nearest integer */
    ...
    u32Period = (u32Tmp1 * sLLSIConfig->sTimeInfo.u32TransferTimeNsec / u32Tmp2 + 5) / 10; /* Round to the nearest integer */
    ...
    u32T0H = (u32Tmp1 * sLLSIConfig->sTimeInfo.u32T0HTimeNsec / u32Tmp2 + 5) / 10; /* Round to the nearest integer */
    ...
    u32T1H = (u32Tmp1 * sLLSIConfig->sTimeInfo.u32T1HTimeNsec / u32Tmp2 + 5) / 10; /* Round to the nearest integer */
    ...
    u32ResetPeriod = (u32Tmp1 * sLLSIConfig->sTimeInfo.u32ResetTimeNsec / u32Tmp2 + 5) / 10; /* Round to the nearest integer */
