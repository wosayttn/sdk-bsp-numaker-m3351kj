...
static uint32_t CANFD_CalculateRamAddress(CANFD_RAM_PART_T *psConfigAddr, CANFD_ELEM_SIZE_T *psConfigSize)
{
    ...
    if (psConfigSize->u32TxBuf > 0)
    {
        ...
        u32RamAddrOffset += (psConfigSize->u32TxBuf) * sizeof(CANFD_TX_BUF_T);
    }

    /* Get the Tx Buffer and Tx FIFO/Queue element address */
    if (psConfigSize->u32TxFifoQueue > 0)
    {
        u32RamAddrOffset += (psConfigSize->u32TxFifoQueue) * sizeof(CANFD_TX_BUF_T);
    }
...
void *CANFD_Transfer(CANFD_T *psCanfd, E_CANFD_MSG eCanfdMsg, void *psUserData, uint32_t u32Idx)
{
    ...
    case eCANFD_MSG_DRB: /*!< Dedicated RX Buffer  */
    {
    ...
    case eCANFD_MSG_TFQ: /*!< TX FIFO/QUEUE */
    {
        /* Check if Tx FIFO/Queue is enabled */
        if (((psCanfd->TXBC & CANFD_TXBC_TFQS_Msk) >> CANFD_TXBC_TFQS_Pos) == 0)
            goto EXIT_CANFD_Transfer;

        /* Check if FIFO has free space (TFFL: Tx FIFO Free Level) */
        if (((psCanfd->TXFQS & CANFD_TXFQS_TFFL_Msk) >> CANFD_TXFQS_TFFL_Pos) == 0)
            goto EXIT_CANFD_Transfer;

        if (psUserData)
        {
            uint32_t u32PutIdx = (psCanfd->TXFQS & CANFD_TXFQS_TFQPI_Msk) >> CANFD_TXFQS_TFQPI_Pos;

            CANFD_TX_BUF_T *psDstTxBuf = CANFD_GET_TXBUF_ADDR(psCanfd, u32PutIdx);

            memcpy((void *)psDstTxBuf, psUserData, sizeof(CANFD_TX_BUF_T));

            /* Request append new Tx buffer. */
            psCanfd->TXBAR = (1 << u32PutIdx);
    ...
    case eCANFD_MSG_DTB: /*!< Dedicated TX Buffer  */
    {
